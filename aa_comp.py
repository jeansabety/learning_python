...
for name, seq in mcb185.read_fasta(sys.argv[1]): 
	for aa in seq: 
		if aa == '*' : #if you see the stop codon..
			continue #skip it 
		if aa in count: 
			count[aa] += 1 #if you see it, add one more
		else: 
			count[aa] = 1 #if you havent counted any od that AA yet, make that first AA 1 (will be first time you see it )
		total += 1

#print dictionary in a readable way
for aa in count: 
	print(aa, count[aa], count[aa]/total)


"""
python3 aa_comp.py -- fasta at_prots.fa | sort -nk2 #how you sort it 
